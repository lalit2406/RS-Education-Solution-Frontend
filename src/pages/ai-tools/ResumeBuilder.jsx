...
export default function ResumeBuilder() {
  ...
  const [rsRbLoading, setRsRbLoading] = useState(true);

  useEffect(() => {
    // no initial API → load instantly
    setRsRbLoading(false);
  }, []);
  ...
  const rbGenerateResume = async (e) => {
    ...
    try {
      ...
      const cleanCerts = rbFilteredCertifications(rbForm.certifications);
      ...
      setRbGeneratedResume({
        ...
        certifications: rbFilteredCertifications(rbForm.certifications),
      });
  ...
  const rbDownloadResume = () => {
    const fileText = JSON.stringify(rbGeneratedResume, null, 2);
  ...
  const rbProjects = rbFilteredProjects(rbPreview.projects || []);
  const rbEducation = rbFilteredEducation(rbPreview.education || []);
  const rbCerts = rbFilteredCertifications(rbPreview.certifications || []);

  const rbSkillGroups = [
    {
      title: "Languages",
      values: rbSplitSkills(rbPreview.skills?.languages),
    },
    {
      title: "Web Dev",
      values: rbSplitSkills(rbPreview.skills?.web_dev),
    },
    {
      title: "AI / ML",
      values: rbSplitSkills(rbPreview.skills?.ml_ai),
    },
    {
      title: "Libraries",
      values: rbSplitSkills(rbPreview.skills?.libraries),
    },
    {
      title: "Tools / DB",
      values: rbSplitSkills(rbPreview.skills?.tools_dbs),
    },
    ...
    <div className="rb-page-wrap">
      <div className="rb-page-container">
        <div className="rb-topbar">
          <button className="rb-back-btn" onClick={() => navigate("/ai-tools")}>
            <FaArrowLeft />
            Back to AI Tools
          </button>
        ...
        <div className="rb-hero-card">
          {rsRbLoading ? (
            <>
              <div className="rs-rb-skeleton-title"></div>
              <div className="rs-rb-skeleton-desc"></div>
            </>
          ) : (
            <>
              <div className="rb-hero-left">
                ...
                <p>
                  Smart resume builder with clean structure, recruiter focused
                  layout and modern design.
                </p>
              </div>
        ...
        {rsRbLoading ? (
          <div className="rb-section-card">
            <div className="rs-rb-skeleton-form"></div>
            <div className="rs-rb-skeleton-form"></div>
            <div className="rs-rb-skeleton-form"></div>
            <div className="rs-rb-skeleton-btn"></div>
          </div>
        ) : (
          rbShowForm && (
            <form className="rb-main-form" onSubmit={rbGenerateResume}>
              {/* PERSONAL DETAILS */}
              <section className="rb-section-card rb-form-card">
                <div className="rb-form-head">
                  ...
                  <div>
                    ...
                    <p>Basic information recruiters use to contact you.</p>
                  </div>
                </div>
              ...
              <section className="rb-section-card rb-form-card">
                <div className="rb-form-head">
                  ...
                  <div>
                    ...
                    <p>Add strong keywords for better ATS score.</p>
                  </div>
                </div>
              ...
              <section className="rb-section-card rb-form-card">
                <div className="rb-section-head rb-space-between">
                  <div className="rb-form-head rb-no-margin">
                    ...
                    <div>
                      <h2>Projects</h2>
                      <p>Add strong real work or personal projects.</p>
                    </div>
                  </div>
                ...
                {rbForm.projects.map((item, index) => (
                  <div key={index} className="rb-repeat-card rb-modern-repeat">
                    ...
                    <div className="rb-grid-two">
                      <div className="rb-input-group">
                        ...
                        <input
                          ...
                          value={item.role}
                          onChange={(e) =>
                            rbHandleProjectChange(index, "role", e.target.value)
                          }
                        />
                      </div>
                      ...
                      <div className="rb-input-group">
                        ...
                        <input
                          ...
                          value={item.name}
                          onChange={(e) =>
                            rbHandleProjectChange(index, "name", e.target.value)
                          }
                        />
                      </div>
              ...
              <section className="rb-section-card rb-form-card">
                <div className="rb-section-head rb-space-between">
                  <div className="rb-form-head rb-no-margin">
                    ...
                    <div>
                      <h2>Education</h2>
                      <p>Add latest and relevant qualifications.</p>
                    </div>
                  </div>
                  ...
                  <div key={index} className="rb-repeat-card rb-modern-repeat">
                    <div className="rb-repeat-top">
                      ...
                      <button
                        type="button"
                        className="rb-delete-btn"
                        onClick={() => rbRemoveEducation(index)}
                      >
                        <FaTrash />
                      </button>
                    </div>
              ...
              <section className="rb-section-card rb-form-card">
                <div className="rb-section-head rb-space-between">
                  <div className="rb-form-head rb-no-margin">
                    ...
                    <div>
                      ...
                      <p>Add certificates, internships or awards.</p>
                    </div>
                  </div>
                ...
                {rbForm.certifications.map((item, index) => (
                  <div key={index} className="rb-cert-row">
                    <div className="rb-input-group">
                      ...
                      <input
                        ...
                        value={item}
                        onChange={(e) =>
                          rbHandleCertificationChange(index, e.target.value)
                        }
                      />
                    </div>
                    ...
                    <button
                      type="button"
                      className="rb-delete-btn"
                      onClick={() => rbRemoveCertification(index)}
                    >
                      <FaTrash />
                    </button>
                  </div>
              ...
              <button
                ...
                disabled={rbLoading}
              >
                {rbLoading ? "Generating Resume..." : "Generate Resume"}
              </button>
            </form>
          )
        )}
          ...
          <section className="rb-output-card">
            <div className="rb-section-head rb-space-between">
              ...
              <div className="rb-action-group">
                <button className="rb-copy-btn" onClick={rbCopyResume}>
                  <FaCopy />
                  {rbCopied ? "Copied" : "Copy"}
                </button>

                <button className="rb-copy-btn" onClick={rbDownloadResume}>
                  <FaDownload />
                  Download
                </button>

                <button className="rb-copy-btn" onClick={rbCreateNewResume}>
                  <FaRedo />
                  New Resume
                </button>
              </div>
            ...
            <div className="rb-preview-sheet">
              {/* header */}
              <div className="rb-resume-name">{rbPreview.name}</div>

              <div className="rb-resume-contact">
                {rbHasValue(rbPreview.phone) && (
                  <span>
                    <FaPhoneAlt />
                    {rbPreview.phone}
                  </span>
              ...
              {/* summary */}
              {rbHasValue(rbPreview.summary) && (
                <>
                  <div className="rb-preview-section">Professional Summary</div>

                  <p className="rb-summary-box">{rbPreview.summary}</p>
                </>
              )}

              {/* skills */}
              {rbSkillGroups.length > 0 && (
                <>
                  <div className="rb-preview-section">Technical Skills</div>

                  <div className="rb-skill-wrap">
                    {rbSkillGroups.map((group, index) => (
                      <div key={index} className="rb-skill-group">
                        <div className="rb-skill-title">{group.title}</div>

                        <div className="rb-badge-row">
                          {group.values.map((item, i) => (
                            <span key={i} className="rb-skill-badge">
                              {item}
                            </span>
                          ))}
                        </div>
              ...
              {rbProjects.length > 0 && (
                <>
                  <div className="rb-preview-section">Projects</div>

                  {rbProjects.map((item, index) => (
                    <div key={index} className="rb-pro-card">
                      {(rbHasValue(item.name) || rbHasValue(item.role)) && (
                        <div className="rb-pro-head">
                          {item.name}
                          {rbHasValue(item.role) && ` | ${item.role}`}
                        </div>
                      )}
                      ...
                      {rbHasValue(item.tech_stack) && (
                        <div className="rb-preview-line">
                          <strong>Tech:</strong> {item.tech_stack}
                        </div>
                      )}

                      {rbHasValue(item.impact) && (
                        <div className="rb-impact-line">{item.impact}</div>
                      )}
                    </div>
                  ))}
                </>
              ...
              {/* education */}
              {rbEducation.length > 0 && (
                <>
                  <div className="rb-preview-section">Education</div>

                  {rbEducation.map((item, index) => (
                    <div key={index} className="rb-edu-card">
                      <div className="rb-pro-head">{item.degree}</div>

                      <div className="rb-preview-line">{item.institution}</div>

                      <div className="rb-preview-line">
                        {item.year}{" "}
                        {rbHasValue(item.score) && `| ${item.score}`}
                      </div>
                    </div>
                  ))}
                </>
              ...
              {rbCerts.length > 0 && (
                <>
                  <div className="rb-preview-section">Certifications</div>

                  <div className="rb-badge-row">
                    {rbCerts.map((item, index) => (
                      <span key={index} className="rb-skill-badge">
                        {item}
                      </span>
                    ))}
                  </div>
